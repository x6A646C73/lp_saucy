...
list_search( int *list, int key, int len, int *ndx )
{
    for( *ndx = 0; *ndx < len; ++(*ndx)){
        if( list[*ndx] == key ) /* TODO: need something other than ==? */
            return 1;
    }
...
amorph_read(const char *filename, int digraph)
{
    ...
    int tempk, tempj;
    int w, *wout, *win, *wlist; /* weight data */
    int ndx;
    ...
    wout = malloc(2 * e * sizeof(int)); /* weight data */
    wlist = calloc( 2 * e, sizeof(int) );
    ...
    win = wout + (digraph ? e : 0); /* weight data */

    /* Initial coloring with provided splits */
    /*
    //TODO: figure out how the colors are read/stored
    //      n-vector holding the color of each vertex
    //      set it up to read appropriately
    */
    /*
    for (i = j = 0; i < p - 1; ++i) {
    ...
    */
    /*
    //TODO: determine if this is reasonable, will work with color code
    //      think this is fine, just assumptions about nature of graph
    //      made in original code, I think
    */
    for( i = 0; i < n; i++){
    ...
    for (i = 0; i < e; ++i) {
        ...
        */
        ...
        /*
        //TODO: make sure this doesn't break things (details of ++ operator)
        //      pretty sure the ++ applies after the access
        //      use - temp = aout[j]; eout[temp] = k; aout[j]++;
        //      actually, this might be fine, check!
        */
        tempj = aout[j]++; tempk = ain[k]++;
        eout[tempj] = k; ein[tempk] = j;
        /* Collect weight data */
        /* TODO: probably more like, wout = (w != NULL) ? w : 1; */
        if( !list_search( wlist, w, g->sg.w, &ndx ) ){
            wlist[ndx] = w;
            ++(g->sg.w);
        }
