...
static int check_mapping( struct saucy *s, const int *adj, const int *edg, const int *wght, int k )
{ //{{{
    int i, gk, ret = 1;
...
static int refine_cell( struct saucy *s, struct coloring *c,
...
static int ref_singleton( struct saucy *s, struct coloring *c,
...
} //}}} END ref_nonsingle_cell

static int ref_nonsingle( struct saucy *s, struct coloring *c,
    const int *adj, const int *edg, const int *wght, int cf )
{ //{{{
    int i, j, k, temp;
    int wcount = 0;
...
{ //{{{
    ...
    { //{{{
        ...
        *rmin = s->right.unlab[k];
        ...
        if( clen[*target] && in_cell_range( &s->left, *lmin, *target )
...
void saucy_search(
    ...
    void *arg,
    struct saucy_stats *stats,
    const int *thprev, const int *thnext, const int *thfront,
    const int *threp, const int *thsize, const int *theta );
{ //{{{
    int i, j, max = 0;
    ...
    for( i = 0; i < s->n; ++i ) s->theta[i] = theta[i];
    
    /* The initial permutation is the identity */
    for( i = 0; i < s->n; ++i ) s->gamma[i] = i;
    
    /* Initially every cell of theta has one element */
    for( i = 0; i < s->n; ++i ) s->thsize[i] = thsize[i];
    for( i = 0; i < s->n; ++i ) s->thfront[i] = thfront[i];
    for( i = 0; i < s->n; ++i ) s->threp[i] = threp[i];
