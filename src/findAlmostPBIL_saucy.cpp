...
void initialize( string filename )
{
    ...
    numEdges = ng->sg.e;
    edge_prob = (edge_t *)malloc( numEdges*sizeof(edge_t) );
...
void generate()
{
    ...
    for( i = 0; i < POPSIZE; i++ )
    {
        ...
        //TODO: do as below, just continue looping through the edge list until
        //      the entire budget 'edges' is used up
        //      allow duplicates, each time an edge is chosen, subtract 1 from
        //      its weight
        //      on update, apply update rule to edge as many times as it was
        //      chosen in the best member of the population...
        edges = i4_uniform_ab( 0, K );
        start = i4_uniform_ab( 0, numEdges-1 );
        ...
        for( idx = start, temp = 0; idx < numEdges && temp < edges; idx++ )
        {
            ...
            if( rnum < edge_prob[idx].prob )
            {
                ...
                pop[i].gene[temp].wght = ng->sg.wght[idxw];
                temp++;
            }
        }
